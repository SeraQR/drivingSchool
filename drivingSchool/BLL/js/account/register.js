...
account.blur(() => {
	...
	if (user.act !== '' && !user.rgtAct) {
		...
		if (!regBox.Email.test(user.act) && !regBox.Mobile.test(user.act)) {
			...
			Account.hasInfo('account', 'student', '', user.act) ||
			Account.hasInfo('account', 'coach', '', user.act)
		) {
			user.rgtAct = false
			showTip('账号', '账号已经存在！')
		} else {
...
jQname.blur(() => {
	...
	if (user.name !== '' && !user.rgtName) {
		tipName.hide('slow')
		if (
			Account.hasInfo('name', 'student', user.name) ||
			Account.hasInfo('name', 'coach', user.name)
		) {
			user.rgtName = false
			showTip('昵称', '昵称已经存在！')
		} else {
