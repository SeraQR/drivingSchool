...
;(function($) {
	...
	$.fn.freezeHeader = function(params) {
		...
		function freezeHeader(elem) {
			...
			if (elem.length > 0 && elem[0].tagName.toLowerCase() === 'table') {
				...
				if (params && params.height) {
					obj.divScroll = `<div id="hdScroll${obj.id}" style="height: ${
						params.height
					}; overflow-y: scroll">`
				...
				obj.scroller =
					params && params.height ? $(`#hdScroll${obj.id}`) : $(window)
		...
		function limiteAlcancado(obj, aparams) {
			...
			return (
				$(document).scrollTop() > obj.header.offset().top &&
				$(document).scrollTop() <
					obj.grid.height() -
						obj.header.height() -
						obj.grid.find('tr:last').height() +
...
export function fixedTHeader() {
	$('.table').freezeHeader({ height: '400px' })
}
